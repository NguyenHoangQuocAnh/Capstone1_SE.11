...
class TicketController {
  ...
  async storeTicket(req, res) {
    ...
    const updateSeatQuery = `update Seats set seat_status = 2 where seat_number='${seat_number}'`;
    try {
  ...
  async generateTicket(req, res) {
    let { trip_id, customer_id, departure_date, seat, price } = req.body;
    ...
    const updateSeatQuery = `update Seats set seat_status = 2 where seat_number in (${seat.map(el=>`'${el}'`).join(
      ','
    )})`;
    try {
      await Promise.all([db.query(query), db.query(updateSeatQuery)]);
      res.send({
        status: 200,
        data: ticket_id_arr,
      });
